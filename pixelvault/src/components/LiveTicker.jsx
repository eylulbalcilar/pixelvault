...
const LiveTicker = () => {
  ...
  useEffect(() => {
    // REVIEW: No try/catch around this fetch. If the CoinGecko API is down or rate-limits you,
    // res.json() or the property accesses (data.bitcoin.usd) will throw, and the interval will
    // keep retrying with unhandled errors every 30 seconds.
      ...
      <div className="ticker-track">
        {[...prices, ...prices].map((coin, i) => (
          <div key={i} className="ticker-item">
            ...
            <span
              className={`ticker-change ${coin.change >= 0 ? "up" : "down"}`}
            >
